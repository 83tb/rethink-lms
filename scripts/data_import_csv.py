...
def setup_db(db_name="engine", tables=['lamps', ]):
    ...
    try:
        ...
        # r.db(db_name).table(
        #    'lamps').index_create('location', geo=True).run(connection)
...
def returnLampObj(id, coords, xy):
    location = {
        "type": "Point",
        "coordinates": coords
    }
...
with open('../local/lamps.csv', 'rb') as csvfile:
    ...
    for row in spamreader:
        ...
        for lid in row:
            ...
            if(lid):
                ...
                db_lamps.insert(returnLampObj(lid, coords, xy)).run(c)
            rowItem += 1
        rowN += 1
...
#     json.dump(
#         jsonData, outfile, sort_keys=True, indent=4, ensure_ascii=False)
